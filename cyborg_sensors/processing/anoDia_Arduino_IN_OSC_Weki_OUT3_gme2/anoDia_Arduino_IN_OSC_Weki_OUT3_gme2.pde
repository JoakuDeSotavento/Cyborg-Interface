...
void setup() {
  ...
  remoteWekiAudio = new NetAddress("127.0.0.1", 6000);
  /// address for arduino
  myRemoteArduino = new NetAddress("192.168.43.251", 888);

  //// address for resolume
  myRemoteReso = new NetAddress("127.0.0.1", 57120);
...
void outputData() {
  pushStyle();
  fill(#e66607);
  float processHeight0 = map(p0, 0, 10, 0, 200);
  rect(540, 250, 50, -processHeight0);
  float processHeight1 = map(p1, 0, 10, 0, 200);
  rect(610, 250, 50, -processHeight1);
  float processHeight2 = map(p2, 0, 10, 0, 200);
  rect(680, 250, 50, -processHeight2);
  float processHeight3 = map(p3, 0, 10, 0, 200);
  rect(750, 250, 50, -processHeight3);
  float processHeight4 = map(p4, 0, 10, 0, 200);
  rect(810, 250, 50, -processHeight4);
  float processHeight5 = map(p5, 0, 10, 0, 200);
  rect(880, 250, 50, -processHeight5);
  float processHeight6 = map(p6, 0, 10, 0, 200);
