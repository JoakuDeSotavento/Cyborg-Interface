void sendOSC(float _p0, float _p1, float _p2, float _p3, float _p4, float _p5, float _p6, boolean _bunOrMess) {
  if (!_bunOrMess) {
    ...
    myMessage.setAddrPattern("/composition/selectedlayer/transport/position/behaviour/speed");
    myMessage.add((float)_p1);
    myBundle.add(myMessage);
    myMessage.clear();
    /* refill the osc message object again */
    myMessage.setAddrPattern("/composition/selectedlayer/transport/position/behaviour/playdirection");
    ...
    myMessage.clear();
    /* refill the osc message object again */
    myMessage.setAddrPattern("/composition/selectedlayer/video/effects/transform/positionx");
    myMessage.add((float)0.5);
    myBundle.add(myMessage);
    myMessage.clear();
    /* refill the osc message object again */
    myMessage.setAddrPattern("/composition/selectedlayer/video/effects/transform/positiony");
    myMessage.add((float)0.5);
    myBundle.add(myMessage);
    myMessage.clear();
    /* refill the osc message object again */
    myMessage.setAddrPattern("/composition/selectedlayer/video/effects/transform/scale");
